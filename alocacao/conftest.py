...
# E as fixtures de sessao, que a guarda vigia pelo mesmo desenho (B-2).
...
# B-2: as duas fixtures de SESSAO eram protegidas so pela docstring "NAO altere" --
...
def custos_originais():
    """Carregado uma vez. NAO altere — use `custos` se precisar alterar. Vigiado (B-2)."""
    return _vigiar("custos_originais", carregar_custos())


@pytest.fixture(scope="session")
def politica_original():
    """Carregada uma vez. NAO altere — use `politica` se precisar alterar. Vigiada (B-2)."""
...
def guarda_de_estado_compartilhado(request):
    ...
    mod = request.module
    # B-2: so as fixtures de sessao que ESTE teste recebeu -- quem nao as pediu nao
