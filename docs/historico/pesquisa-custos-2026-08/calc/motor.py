...
def val(no, *, permitir_parcial=True, contexto=""):
    ...
    if exp and isinstance(exp, dt.date) and HOJE > exp:
        print(f"  [AVISO] {contexto}: valor expirou em {exp} (fonte: {no.get('fonte','?')})", file=sys.stderr)
    return no["valor"]
...
def montar_rotas(C):
    """Monta as rotas, capturando InsumoBloqueado como marcador em vez de abortar."""
    ...
    try:
        ...
        add(grupo="ETF na B3", nome="BOVV11 — corretora zero", b3_vista=True, adm_aa=adm, custodia_rv=True)
    except InsumoBloqueado as e:
    ...
    try:
        ...
        add(grupo="Acao individual", nome="XP swing trade", b3_vista=True, corr_fix=xp, custodia_rv=True)
    except InsumoBloqueado as e:
    ...
    add(grupo="Exterior", nome="Rota Wise como conta",
        entrada_pct=val(C["exterior"]["spread_wise"], contexto="wise")+val(C["exterior"]["iof_conta"], contexto="iof.conta"),
