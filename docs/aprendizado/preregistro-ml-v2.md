# Pré-registro — família `aprendizado`, versão 2

*Escrito em 19/09/2026, antes de qualquer variável da base PIT existir. É a fase ML-0 de
`pesquisa-e-plano-ml-1.md`. Corrigido em 21/09/2026 antes do primeiro commit (§13), e **`v2` em
23/09/2026** porque o insumo que motivou o corte de período foi CONSERTADO (§14). A `v1` fica
como foi commitada em `45405a6` — nada medido sob ela muda de versão.*
...
- **Período:** jan/2010 a **ago/2026** (fundamentos DFP desde 2010; COTAHIST inteiro em R$,
  fora da quebra de 1994 — C-03). **2026 entra** (`v2`, §14): o `COTAHIST_A2026.ZIP` foi
  rebaixado íntegro em 23/09/2026 — 85.779.964 bytes, `sha256 fb3546ed27cc…`, diretório
  central presente, cabeçalho `00COTAHIST.2026BOVESPA 20260918`, trailer declarando
  **2.871.743** registros e 2.871.743 lidos, **179 pregões de 02/01 a 18/09/2026**. O último
  mês fechado do acervo é **ago/2026**, e é onde o teste termina.
...
| séries `INCOMPLETO` por provento sem preço de véspera (P-113) | indeterminada | **fechada em 23/09**: o fechamento do COTAHIST substitui a ausência, com coluna de procedência; `INCOMPLETO` 78 → 74 |
| o NEFIN termina em **03/07/2026** | nenhuma no alvo; atinge só a métrica secundária | o alfa contra fatores (§6) é calculado até **jun/2026** e o relatório diz isso ao lado do número; os dois meses finais entram no IC e ficam fora do alfa |
...
- **Teste:** jan/2020 a **ago/2026 — 80 meses**. Os modelos são retreinados anualmente com
...
sem bancos; subperíodos 2020–2022 e 2023–2026. **Robustez que inverte o sinal rebaixa o
...
só pela aritmética (`c / √80`): com corte 3,1, o IC médio precisa chegar a **0,35 do desvio
mensal**; com 4,1, a **0,46**. Se o poder medido for baixo, um "não rejeita" será lido como
...
- **Mudança de regime depois de 2026.** É o que a sombra e o CUSUM existem para detectar, não
...
| 2, 4 | período até ago/2026; teste de 80 meses | até dez/2025; teste de 72 meses — **DESFEITO pela `v2`, §14** | 2026 truncado (P-100), e o truncamento era conserto, não limitação |
...
## 14. Por que existe uma `v2` (23/09/2026)

**A `v1` cortou o período em dez/2025 por um defeito meu de método, não por um fato do mundo.**
O `COTAHIST_A2026.ZIP` tinha chegado truncado em 18/09 (P-100), e eu tratei **dado ausente por
problema técnico como se fosse limitação do mundo**: escrevi "2026 fica fora" em vez de
"o arquivo tem de ser baixado de novo". Ele baixou de novo em 23/09 e o arquivo veio **íntegro**.

> **A regra que sai daqui, e ela é maior que este documento (§5-B.16):** *ausência de dado por
> problema técnico é pendência de conserto, nunca limitação declarada.* `limitacoes_declaradas`
> é para o que o mundo não fornece — não para o que ninguém tentou de novo. Declarar um defeito
> como limitação **encerra a investigação** e fica com cara de rigor, que é a mesma forma da
> §5-B.13.

| | `v1` (commit `45405a6`) | `v2` |
|---|---|---|
| período | jan/2010 a dez/2025 | **jan/2010 a ago/2026** |
| teste | jan/2020 a dez/2025, 72 meses | **jan/2020 a ago/2026, 80 meses** |
| blocos em L = 2 / L = 3 | 36 / 24 | **40 / 27** |
| efeito mínimo detectável (aritmética) | 0,37 / 0,48 do desvio | **0,35 / 0,46** |
| subperíodo de robustez | 2023–2025 | **2023–2026** |
| P-100 | bloqueava a família | **fechada — arquivo íntegro medido** |
| NEFIN até 03/07/2026 | não declarado | **declarado**: alfa contra fatores até jun/2026 |

**O que NÃO mudou:** hipóteses, `m` (11 e 24), variáveis, modelos, grade, critério da §7,
os nove testes da §8, o plano de mudanças e o modo sombra. Uma `v2` que mudasse isso seria
outra pesquisa; esta só devolve o período que o defeito havia tirado.

**Vale a partir do commit empurrado que a contém**, como a `v1`. E a `v1` continua valendo para
o que foi medido sob ela — nada foi, porque nenhuma variável da ML-3 existe ainda.
